...
public class BibliographyManager {
    ...
    public static void add(List<BibliographyEntry> bibliographyEntries) {
        bibliographyEntries.forEach( b ->{
            try {
                String sql = "insert into bibliographyentry(IDCategory, Author, Title, NumberOfPages, Year, Cost) " +
                             "values ('" + b.getIdCategory().toString() + "', '" + b.getAuthor() +"', '" + b.getTitle() +"', '" + b.getNumberOfPages().toString() +"', '" + b.getYear().toString() +"', '" + b.getCost().toString() + "');";
                PreparedStatement statement = DBManager.getConnection().prepareStatement( sql );
                statement.execute();
            } catch (SQLException e) {
                e.printStackTrace();
